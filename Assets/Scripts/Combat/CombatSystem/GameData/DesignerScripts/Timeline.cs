    ...
    public class Timeline{
        ...
        private static void FireBullet(TimelineObj tlo, params object[] args){
            ...
            if (tlo.caster){
                ...
                int muzzleId = bLauncher.model.muzzleId;
                int soundId = bLauncher.model.soundId;
                if (muzzleId >= 0)
                {
                    GameEntry.Entity.ShowEffectEntity(muzzleId, ubp.transform.position, ubp.transform.rotation, new EffectEntityUserData(3));
                    
                }

                if (soundId >= 0)
                {
                    GameEntry.Sound.PlaySFX3D(soundId, ubp.transform.position);
                }
        ...
        private static void Play2DSound(TimelineObj tlo, params object[] args){
            if (tlo.caster){
                var soundId = args[0];
                GameEntry.Sound.PlaySFX2D((int)soundId);
            }
        }
        ...
        private static void Play3DSound(TimelineObj tlo, params object[] args){
            if (tlo.caster)
            {
                ...
                if (bindManager)
                {
                    ...
                    GameEntry.Sound.PlaySFX3D((int)soundId, trans.position);
                }
        ...
        private static void PlayEffectInWorld(TimelineObj tlo, params object[] args)
        {
            ...
            var rotation = tlo.caster.transform.rotation*(Quaternion)args[2];

            
            var effectEntity = GameEntry.Entity.ShowEffectEntity((int)effectTypeId, pos, (Quaternion)rotation,null);
        ...
        private static void SummonCharacter(TimelineObj timelineObj, params object[] args){
            ...
            string campType = args.Length > 1 ? (string)args[1] : "Unknown";
