...
public class EquipmentProps
{
    ...
    public EquipmentProps(ChaProperty[] propAdd)
    {
        this.propMod = propAdd;
    }
}
...
public class ChaState:MonoBehaviour{
    ...
    public Quaternion MoveRotation{
        get{
            return wishToMoveRotation;
        }
    }
    private Quaternion wishToMoveRotation ;
    ...
    public Quaternion RotateRotation{
        get{
            return wishToRotateRotation;
        }
    }
    private Quaternion wishToRotateRotation;
    ...
    public Action onControlStateChange;
    private ChaProperty lastProperty;
    private ChaControlState lastControlState;

    public Action<string, int, float,bool,bool,bool,bool> onBuffChange;
    
    [SerializeField]
    private string camp = "";//角色所处阵营，用string标识，通过GameEntry.Combat的BeFriends，BeEnemies等方法来决定战略关系

    public Action<string> onCampChange;
    //阵营
    public string Camp
    {
        get => camp;
    ...
    public float MoveSpeed{get{
        ...
        return this._prop.moveSpeed /100f;
    }}
    ...
    public float ActionSpeed{
        get{
            return this._prop.actionSpeed /100f;
        }
    }
    ...
    //装备，不控制装备，只获取装备数据进行属性计算，通过AddWeaponProps，RemoveWeaponProps来增删
    public List<EquipmentProps> weaponsProps = new List<EquipmentProps>();
    public List<EquipmentProps> armorsProps = new List<EquipmentProps>();
    ...
    private List<BuffObj> toRemove = new List<BuffObj>();//提前声明避免每次都新建List
    void FixedUpdate() {
        ...
        if (dead == false){
            ...
            //无敌时间减少
            if (_immuneTime > 0) _immuneTime -= fixedDeltaTime;

            //技能冷却时间
            for (int i = 0; i < this.skills.Count; i++){
                if (this.skills[i].cooldown > 0){
                    this.skills[i].cooldown -= fixedDeltaTime;
                }
            }

            //对身上的buff进行管理
            toRemove = new List<BuffObj>();
            for (int i = 0; i < this.buffs.Count; i++){
                if (buffs[i].permanent == false) buffs[i].duration -= fixedDeltaTime;
            ...
            if (toRemove.Count > 0){
                for (int i = 0; i < toRemove.Count; i++){
                    onBuffChange?.Invoke(toRemove[i].model.id,toRemove[i].stack,0,false,toRemove[i].model.stateMod.canMove,toRemove[i].model.stateMod.canRotate,toRemove[i].model.stateMod.canUseSkill);
            ...
            if (wishToMove == true) 
                wishToMoveRotation = Quaternion.LookRotation(moveOrder);
            ...
            if (unitMove){
                ...
                while (fmIndex < forceMove.Count){
                    moveOrder += forceMove[fmIndex].VeloInTime(fixedDeltaTime);
                ...
                unitMove.MoveBy(moveOrder,Time.fixedDeltaTime);
                moveOrder = Vector3.zero;
                
            
            }
            
            wishToRotateRotation = rotateToOrder;
            if (wishToMove == false) wishToMoveRotation = wishToRotateRotation;
            //然后是旋转信息
            if (unitRotate)
            {
                ...
                unitRotate.RotateTo(rotateToOrder,Time.fixedDeltaTime); 
                forceRotate.Clear();//forceRotate和forceMove不一样,forceMove添加的是持续效果，forceRotate添加的是一帧内的旋转
            }
            //再是动画处理
            if (unitAnim){
            ...
            wishToRotateRotation = transform.rotation;
            wishToMoveRotation = wishToRotateRotation;
        }
    }
    ...
    private void AttrRecheck(){
        ...
        for (var i = 0; i < equipmentProp.Length; i++) equipmentProp[i].Zero();

        for (int i = 0; i < weaponsProps.Count; i++)
        {
            for (int j = 0; j < Mathf.Min(equipmentProp.Length, weaponsProps[i].propMod.Length); j++){
                equipmentProp[j] += weaponsProps[i]. propMod[j] ;
            }
        }
        
        for (int i = 0; i < armorsProps.Count; i++)
        {
            for (int j = 0; j < Mathf.Min(equipmentProp.Length, armorsProps[i].propMod.Length); j++){
                equipmentProp[j] += armorsProps[i]. propMod[j] ;
            }
        }
        ...
        if (!this._prop.Equals(lastProperty))
        {
            onPropChange?.Invoke();
            lastProperty = this._prop;
        }

        if (!this._controlState.Equals(lastControlState))
        {
            onControlStateChange?.Invoke();
            lastControlState = this.ControlState;
        }
    }
    ...
    public void ModResource(ChaResource value)
    {
        ...
        this.resource.oxygen = Mathf.Clamp(this.resource.oxygen, 0, 100);
        onResourceChange?.Invoke();//用于血条更新之类的
    ...
    public void PlaySightEffect(string bindPointKey, int typeId, string effectKey = "", bool loop = false){
        bindPoints.AddBindEffect(bindPointKey, typeId, effectKey, loop);
    }
    ...
    public void StopSightEffect(string bindPointKey, string effectKey){
        ...
        bindPoints.RemoveBindEffect(bindPointKey, effectKey);
    }
    ...
    public void AddBuff(AddBuffInfo buff){
        ...
        if (hasOnes.Count > 0){
            ...
            onBuffChange?.Invoke(hasOnes[0].model.id,hasOnes[0].stack,hasOnes[0].duration,hasOnes[0].permanent,hasOnes[0].model.stateMod.canMove,hasOnes[0].model.stateMod.canRotate,hasOnes[0].model.stateMod.canUseSkill);
            ...
            buffs.Add(toAddBuff);
            onBuffChange?.Invoke(toAddBuff.model.id,toAddBuff.stack,toAddBuff.duration,toAddBuff.permanent,toAddBuff.model.stateMod.canMove,toAddBuff.model.stateMod.canRotate,toAddBuff.model.stateMod.canUseSkill);
            buffs.Sort((a, b)=>{
