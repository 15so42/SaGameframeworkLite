...
namespace DesignerScripts
{
    ...
    public class AoE{
        ...
        public static Dictionary<string, AoeOnCharacterEnter> onChaEnterFunc = new Dictionary<string, AoeOnCharacterEnter>(){
            ...
            {"DoHealToEnterCha", DoHealToEnterCha},
        ...
        private static AoeMoveInfo AroundCaster(GameObject aoe, float t){
            ...
            return new AoeMoveInfo(UnitMoveType.fly, targetP, Quaternion.Euler(0,cDeg % 360,0));
        }
        ...
        private static AoeMoveInfo FollowCaster(GameObject aoe, float t)
        {
            AoeState aoeState = aoe.GetComponent<AoeState>();
            if (aoeState == null || aoeState.caster == null) return null;
            return new AoeMoveInfo(UnitMoveType.ground, aoeState.caster.transform.position - aoe.transform.position,
                Quaternion.identity);
        }
        ...
        private static AoeMoveInfo RandomMoveWithEasing(GameObject aoe, float t)
        {
            ...
            // 返回相对于原始位置的偏移量
            return new AoeMoveInfo(UnitMoveType.ground, moveDelta, Quaternion.identity);
        }
        ...
        private static AoeMoveInfo FollowCasterIgnoreY(GameObject aoe, float t)
        {
            ...
            dir.y = 0;
            return new AoeMoveInfo(UnitMoveType.ground, dir,
                Quaternion.identity);
        }

        private static AoeMoveInfo FloatUp(GameObject aoe, float t)
        {
            return new AoeMoveInfo(UnitMoveType.ground, Vector3.up*3*Time.fixedDeltaTime,
                Quaternion.identity);
        }
        ...
        private static void BlockBullets(GameObject aoe, List<GameObject> bullets){
            ...
            string ccsCamp = GameEntry.Const.CONST_Camp_Unknown;
            if (aoeState.caster){
            ...
            for (int i = 0; i < bullets.Count; i++){
                ...
                string bCamp = GameEntry.Const.CONST_Camp_Unknown;
                if (bs && bs.caster){
                    ChaState bcs = bs.caster.GetComponent<ChaState>();
                    if (bcs) bCamp = bcs.Camp;
                }
                if (GameEntry.Combat.GetRelation(ccsCamp,bCamp)!=RelationType.Friendly){
                    GameEntry.Combat.RemoveBullet(bullets[i].GetComponent<BulletState>(), false);
                    GameEntry.Combat.CreateSightEffect(70000, aoe.transform.position, aoe.transform.rotation);
                }
            }
        ...
        private static AoeMoveInfo SpaceMonkeyBallRolling(GameObject aoe, float t){
            ...
            return new AoeMoveInfo(UnitMoveType.fly, velocity, Quaternion.LookRotation(velocity));
        }
        ...
        private static void DoDamageToEnterCha(GameObject aoe, List<GameObject> characters){
            ...
            float damageTimes = p.Length > 1 ? (float)p[1] : 1;
            ...
            bool disableMove = p.Length > 8 ? (bool)p[8] : false;
            ...
            string camp = GameEntry.Const.CONST_Camp_Unknown;
            ChaState ccs=null;
            if (aoeState.caster){
            ...
            for (int i = 0; i < characters.Count; i++){
                ...
                //背后检测,有ForceDamageIgnoreBehindCheck这个buff的时候免疫背后攻击
                ...
                if (cs && cs.dead == false && ((toFoe == true && GameEntry.Combat.GetRelation(camp,cs.Camp)!=RelationType.Friendly) || (toAlly == true && GameEntry.Combat.GetRelation(camp,cs.Camp)==RelationType.Friendly))){
                    ...
                    if (soundStrArr.Length > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(soundIdStr))
                        {
                            ...
                            if (soundId >=0) GameEntry.Sound.PlaySFX3D(soundId, cs.transform.position);
                        }
                    }
        ...
        private static void DoHealToEnterCha(GameObject aoe, List<GameObject> characters){
            AoeState aoeState = aoe.GetComponent<AoeState>();
            if (!aoeState) return;
            ...
            aoeState.param.TryGetValue("soundIds", out var soundIdsParam);
            aoeState.param.TryGetValue("Camp", out var campParam);//特殊情况下使用，比如爆炸是在死亡后创造aoe，那爆炸Aoe就不知道创造者是谁，也不知道阵营，所以需要这里传入阵营
            aoeState.param.TryGetValue("DisableMove", out var disableMoveParam);
            ...
            for (int i = 0; i < characters.Count; i++){
                ...
                if (cs && cs.dead == false && ((toFoe == true && GameEntry.Combat.GetRelation(camp,cs.Camp)!=RelationType.Friendly) || (toAlly == true && GameEntry.Combat.GetRelation(camp,cs.Camp)==RelationType.Friendly))){
                    ...
                    GameEntry.Combat.CreateDamage(
                        aoeState.caster, characters[i], 
                        damage*-1, 
                        0.05f, new DamageInfoTag[]{DamageInfoTag.directHeal}
                    );
                    //cs.AddBuff(new AddBuffInfo(DesingerTables.Buff.data["Poison"],aoeState.caster,cs.gameObject,1,10));
                    if (hurtAction == true)
                    {
        ...
        public static void KnockOffEnterCha(GameObject aoe, List<GameObject> characters)
        {
            ...
            for (int i = 0; i < characters.Count; i++){
                ...
                if (  cs && cs.dead == false && ((toFoe == true && GameEntry.Combat.GetRelation(camp,cs.Camp)!=RelationType.Friendly) || (toAlly == true && GameEntry.Combat.GetRelation(camp,cs.Camp)==RelationType.Friendly)))
                {
                   ...
                   UnityTimer.Timer.Register(0.3f, () =>
                    {
                        if (cs != null)
                        {
        ...
        private static void DoDamageOnRemoved(GameObject aoe){
            ...
            for (int i = 0; i < aoeState.characterInRange.Count; i++){
                ...
                if (cs && cs.dead == false && ((toFoe == true && GameEntry.Combat.GetRelation(ccsCamp,cs.Camp)!=RelationType.Friendly) || (toAlly == true && GameEntry.Combat.GetRelation(ccsCamp,cs.Camp)==RelationType.Friendly))){
        ...
        private static void CreateSightEffect(GameObject aoe){
            ...
            GameEntry.Combat.CreateSightEffect(
                fxTypeId, aoe.transform.position, aoe.transform.rotation
            );
        }
        ...
        private static void BarrelExplosed(GameObject aoe){
            ...
            Damage baseDamage = new Damage(0, 0,1);
            float damageTimes = 1f;
            int effect = 70000;
            string bp = "Body";
            ...
            for (int i = 0; i < aoeState.characterInRange.Count; i++){
                ChaState cs = aoeState.characterInRange[i].GetComponent<ChaState>();
                if (cs && cs.dead == false && GameEntry.Combat.GetRelation(ccsCamp,cs.Camp)!=RelationType.Friendly){
                    if (cs.HasTag("Barrel") == true){
                        GameEntry.Combat.CreateDamage(
                            (GameObject)aoeState.param["Barrel"], aoeState.characterInRange[i],
                            new Damage(0, 9999),  0f, new DamageInfoTag[]{DamageInfoTag.directDamage}
                        );
                    }else{
