...
public class AoeState : MonoBehaviour{
    ...
    ///<summary>
    ///aoe移动轨迹函数,aoe一般不需要重力效果，所以就不高什么AoeCondition了
    ///</summary>
    ...
    ///设置移动和旋转的信息，用于执行
    ///</summary>
    public void SetMoveAndRotate(AoeMoveInfo aoeMoveInfo,float deltaTime){
        if (aoeMoveInfo != null){
            if (unitMove){
                _velo = aoeMoveInfo.velocity / Time.fixedDeltaTime;
                unitMove.MoveBy(_velo,deltaTime);
            }
            if (unitRotate){
                unitRotate.RotateTo(aoeMoveInfo.rotation,deltaTime);
            }
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (duration > 0 && tween != null){
            ...
            SetMoveAndRotate(aoeMoveInfo,Time.fixedDeltaTime);
        }
